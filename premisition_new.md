...
- **表单状态与条件渲染 (Conditional Rendering)**：请为表单配置完整的受控状态 (Controlled State) 或使用表单管理库 (如 `react-hook-form`)。针对标注 **[状态连动]** 的栏位，需实作动态显示/隐藏依赖组件的逻辑。

---

## 1. 模块概述
本模块定义了后台管理系统中「修改用户详情」抽屉（Drawer）的功能规格与介面配置。主要提供管理员检视并编辑特定会员的基础帐户资讯、绑定资料，以及各项系统权限与费率设定。
...
- **帐户状态 (Status Badge)**：显示目前帐户状态（例：冻结、正常、停用，建议搭配颜色标示）。
...
### 2.3 页签结构 (Tabs)
采用 `Tabs` 组件切换两大区块：
1. **基本信息 (Basic Information)**：管理会员基础帐户资料。
...
### 3.1 帐户概况 (Account Overview)
表单栏位配置如下：

| 栏位名称 | 建议变数名 | 组件类型 | 说明 / 验证规则 |
| :--- | :--- | :--- | :--- |
| 帐号 | `account` | 唯读文字 (Readonly Text) | 显示用户帐号名称（如：test_user_1） |
| 会员类型 | `memberType` | 唯读 / 下拉选单 (Select) | 显示或设定代理会员等身分 |
| 帐户状态 | `accountStatus`| 单选 / 下拉选单 (Select/Radio)| 选项：正常 (Normal)、冻结 (Frozen)、停用 (Disabled) |
...
| 新增会员 | `allowAddMember` | **[状态连动]** 控制该用户是否具有新增下级会员的权限。**逻辑要求：只有在勾选（开启）此权限时，下方才会动态展开/显示与「新增会员」相关的进阶设定或附属区块。** |
...
## 6. 权限连动控制逻辑 (Permission Control Logic)
本区块定义了 `index.html` 中的权限开关如何连动控制 `content.html` 内各操作入口与区块的显示与隐藏。
底层实作透过 `window.postMessage` 传递目前启用的权限（TRACE_PERM_*）至 `content.html`，并动态在 `document.body` 加上对应的 `hide-*` class 以隐藏 UI 元素。

### 6.1 基本操作权限
| 权限项目 | Tracecode | 控制目标 (隐藏的 UI 元素) |
| :--- | :--- | :--- |
| **修改会员** | `TRACE_PERM_E884` | 操作选单的「编辑用户」入口 |
| **查看会员详情** | `TRACE_PERM_3FEA` | 操作选单的「查看详情」入口 |
| **查看会员联络资讯** | `TRACE_PERM_2ACB` | 用户详情及编辑弹窗内的「联络方式」卡片区块 |
| **会员敏感资讯（密码）** | `TRACE_PERM_16B6` | 操作选单的「修改密码」入口 |
| **修改余额** | `TRACE_PERM_2AD1` | 操作选单的「额度修改」入口 |
| **变更代理** | `TRACE_PERM_068B` | 操作选单的「代理变更」入口 |
| **回访备注** | `TRACE_PERM_DB05` | 操作选单的「回访备注」入口 |

### 6.2 资金与游戏相关权限
| 权限项目 | Tracecode | 控制目标 (隐藏的 UI 元素) |
| :--- | :--- | :--- |
| **资金明细** | `TRACE_PERM_9DB0` | 操作选单的「资金明细」入口 |
| **隐藏资金明细** | `TRACE_PERM_AA83` | 操作选单的「隐藏资金明细」入口 |
| **注单明细** | `TRACE_PERM_0B6F` | 操作选单的「注单明细」入口 |
| **第三方游戏** | `TRACE_PERM_76B4` | 操作选单的「第三方游戏」入口 |
| **交易设定** | `TRACE_PERM_E854` | 操作选单的「交易设定」入口 |
| **会员赔率设置-查看** | `TRACE_PERM_8E99` | 操作选单的「赔率设置」入口 |
| **积分调整** | `TRACE_PERM_5546` | 操作选单的「积分修改」入口 |
| **用户钱包展示** | `TRACE_PERM_FBF8` | 操作选单的「链上地址」入口 |
| **额度修改(链上充值)** | `TRACE_PERM_E91D` | 操作选单的「额度修改(链上充值)」入口 |

### 6.3 下级与纪录相关权限
| 权限项目 | Tracecode | 控制目标 (隐藏的 UI 元素) |
| :--- | :--- | :--- |
| **返点设定** | `TRACE_PERM_F7C1` | 操作选单的「下级会员」、「下级报表」、「下级注单」入口 |
| **清除稽核记录** | `TRACE_PERM_F2E5` | 操作选单的「稽核记录」入口 |
| **代理变更历程** | `TRACE_PERM_37A5` | 操作选单的「代理变更记录」入口 |
| **标签编辑纪录** | `TRACE_PERM_25E8` | 操作选单的「用户标签编辑纪录」入口 |

### 6.4 其他进阶权限
| 权限项目 | Tracecode | 控制目标 (隐藏的 UI 元素) |
| :--- | :--- | :--- |
| **快速登录变更** | `TRACE_PERM_B03A` | 操作选单的「快速登录变更」入口 |
| **会员谷歌验证码查看** | `TRACE_PERM_537E` | 操作选单的「谷歌验证码」入口 |
| **校验用户任务** | `TRACE_PERM_8D00` | 操作选单的「校验用户任务」入口 |
| **编辑标签** | `TRACE_PERM_4F3B` | 操作选单的「编辑标签」入口 |